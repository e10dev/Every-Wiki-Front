<template lang="pug">
.page.page-delete
  p 게시물을 삭제하면 특별한 권한이 있는 사용자만 이 게시물과 게시물의 역사를 열람할 수 있게 됩니다.
  p 게시물 삭제 기록은 공개되므로, 이 위키의 정책에 맞도록 신중하게 진행해 주세요.
  b-field(label="삭제 사유")
...
export default {
  ...
  async asyncData ({ params, query, req, res, error, store }) {
    ...
    try {
      ...
      store.commit('meta/update', {
        title: `"${article.fullTitle}" 게시물 삭제`,
        toolBox: {
      ...
      if (!article.allowedActions.includes('delete')) {
        return error({ statusCode: 403, message: '이 게시물을 삭제할 권한이 없습니다.' })
      }
      return {
        article
      }
